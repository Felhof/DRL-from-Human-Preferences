...
def reward_modelling_process(mocker):
    def _create_reward_modelling_process(
        ...
        mocker.patch("src.rewardmodelling.RewardModel",
                     return_value=reward_model if reward_model is not None else mocker.Mock())

        mocker.patch(
            "src.rewardmodelling.PreferenceBuffer",
            side_effect=[
                training_buffer if training_buffer is not None else mocker.Mock(),
                evaluation_buffer if evaluation_buffer is not None else mocker.Mock(),
            ]
        )

        reward_modeller = RewardModellingProcess(preference_queue, reward_model_queue, stop_queue)
...
def test_reward_modelling_process_puts_initial_reward_model_in_queue(mocker, reward_modelling_process):
    # Given
    ...
    reward_modeller.reward_model_queue.put.assert_called_once_with(
        reward_model
    )


def test_reward_modelling_process_run_gets_all_preferences_from_queue(mocker, runnable_reward_modelling_process):
    # Given
    preference_queue = mocker.Mock()
    ...
    reward_modelling_process = runnable_reward_modelling_process(
        preference_queue=preference_queue, training_buffer=training_buffer,
        evaluation_buffer=evaluation_buffer, use_for_training=[1, 1, 1, 0, 1]
    )
